...
Test.sim = 0; % if you have already save the data as S-VolAngStep4-OC1.mat, 
...
%phase jump
Test.delta_phi          = -13.24;    % phase angle jump in deg
...
Test.r_grid1 = 0.02;
Test.x_grid2 = 0.5; %% SCR = 2, if you use SCR = 10, the value of phase jump should be changed;
Test.r_grid2 = 0.05;
...
% Path to dycov envelope CSV file for Voltage Angle Step test
...
if isfield(Test, 'Plim') && isstruct(Test.Plim)
    ...
    if isfile(dycov_envelope_path)
        try
            ...
            fprintf('Successfully loaded envelope from dycov: %s\n', dycov_envelope_path);
        catch ME
        ...
        fprintf('Note: VolAngStep4 OC1 not available in dycov. Using placeholder limits.\n');
        Test.Plim.t = linspace(0, Test.t_sim, 1000)';
        Test.Plim.p_up = ones(size(Test.Plim.t)) * 1.5;
        Test.Plim.p_down = ones(size(Test.Plim.t)) * (-1.5);
    end
...
% delta_phi  = Test.delta_phi;  % phase angle jump in degrees
% P0         = 0.08;  % initial Power in pu.
% SCR        = 1 /Test.x_grid2;  % SCR = 2 or 10
% D          = 100;   % damping of PT2
% H          = 5;     % inertia (Ta = 2H)
% Xtr        = 0.08;  % transformer reactance
