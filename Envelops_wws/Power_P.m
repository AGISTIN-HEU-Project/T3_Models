%%%% please give here you envelope time t_p and values P_up_dowen

t_p = F1_PhJ_UD.t2;
P_up_down = F1_PhJ_UD.P_up_down;
...
t_begin = max(t(1), t_p(1));

t_end  = min(t(end), t_p(end)+t_begin);
figure(1)
plot(t_p+t_begin,P_up_down,'r')
hold on
plot(t,p,'b');
...
p_up_lim = interp1(t2, P_up_down(:,1), t1, 'linear', 'extrap');
p_down_lim = interp1(t3, P_up_down(:,2), t1, 'linear', 'extrap');
